...
class LogParseWorker(QThread):
    ...
    def run(self):
        ...
        log_pattern = re.compile(
            r"^(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}(?:\.\d{3})?)\s+\[([^\]]+)\]\s+ERROR\s+([\w\.]+)\s+-\s+(.*)"
        )
        ...
        for i, line in enumerate(lines):
            ...
            if match:
                timestamp, thread_name, logger, raw_msg = match.groups()
                ...
                thread_name_esc = escape_cypher(thread_name)

                conn.execute(f"MERGE (t:Thread {{name: '{thread_name_esc}'}})")
                conn.execute(
                    f"MERGE (ex:Exception {{id: '{current_ex_id}', type: '{ex_type_esc}', message: '{ex_msg_esc}', timestamp: timestamp('{timestamp_clean}')}})"
                )
                conn.execute(
                ...
                while j < len(lines):
                    ...
                    if caused_match:
                        ...
                        conn.execute(
                            f"MERGE (c_ex:Exception {{id: '{caused_ex_id}', type: '{c_type_esc}', message: '{c_msg_esc}', timestamp: timestamp('{timestamp_clean}')}})"
                        )
                        conn.execute(
                            f"MATCH (parent:Exception {{id: '{current_ex_id}'}}), (child:Exception {{id: '{caused_ex_id}'}}) MERGE (parent)-[:CAUSED_BY]->(child)"
                        )
                        # Caused by의 예외를 추적 대상으로 전환
                ...
                if call_chain and current_ex_id:
                    # call_chain[0]은 실제 예외가 발생한 최하단(발생지점) 메서드
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Tomcat 로그 자동 분석기 (Kùzu 그래프 인공지능형)")
    ...
    def reset_database(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.btn_upload.setText("📁 catalina.out 파일 선택 및 자동 분석 시작")
    ...
    def setup_ui(self):
        ...
        top_bar = QHBoxLayout()
        self.btn_upload = QPushButton("📁 catalina.out 파일 선택 및 자동 분석 시작")
    ...
    def upload_log(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Tomcat Log File", "", "Log Files (*.out *.log);;All Files (*)"
        )
        if file_path:
            self.btn_upload.setEnabled(False)
            self.btn_upload.setText("⏳ 자동 분석 중... 데이터 파싱 및 그래프 DB 모델링 처리 중")

            # DB 파일 Lock 충돌을 막기 위해 UI 스레드의 DB 연결 해제 후 Worker 시작
            self.close_db_connection()
    ...
    def on_parse_finished(self):
        # Worker 파싱 완료 후 메인 스레드에서 DB 연결 복구
        self.init_database_safely()
    ...
    def run_auto_diagnosis(self):
        ...
        if max_pct == db_pct and db_pct > 0:
            ...
            diagnosis_tag = "⚡ EXTERNAL NETWORK OUTAGE (외부 연동망 장애)"
            recommendation = (
                "   1. [연동사 망 차단]: 외부 결제(PG), 알림톡 API 등 외부 라우팅 유실 장비 다운 여부를 확인하십시오.\n"
                "   2. [타임아웃 타이트닝]: 스레드 동반 결빙 방지를 위해 Read Timeout 설정을 3초 이내로 단축하십시오.\n"
                "   3. [서킷 브레이커 도입]: Resilience4j 같은 시스템 우회 장치 도입을 검토하십시오."
            )
            ...
            recommendation = (
                "   1. [런타임 NullPointer 예외]: 특정 객체의 Null 체크 누락 구간을 보완하십시오.\n"
        ...
        try:
            ...
            for i, iv in enumerate(intervals):
                ...
                bar_str = "■" * bar_length
                lbl_time = iv["start"].strftime("%H:%M")
                chart_section += (
        ...
        detailed_report = (
            ...
            f"   - 결빙된 톰캣 워커 스레드 수 : {total_threads}개\n"
            f"   - 자동 진단 분류 등급 : {diagnosis_tag}\n\n"
            f"{chart_section}\n"
            f" ■ 도메인별 장애 유발 지분율 (RCA 지표)\n"
            f"   ├─ [데이터베이스 영역] : {db_pct}%\n"
            f"   ├─ [외부 연동 네트워크] : {net_pct}%\n"
    ...
    def root_item_clicked(self, item):
        if not self.conn:
            return
        row = item.row()
        target_method = escape_cypher(self.table_root.item(row, 1).text())  # type:ignore
        self.tree_model.clear()
        self.tree_model.setHorizontalHeaderLabels(["에러 전파 타임라인 및 상세 분석 체인"])
