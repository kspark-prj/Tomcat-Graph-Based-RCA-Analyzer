    ...
    def run(self):
        ...
        for i, line in enumerate(lines):
            ...
            if match:
                ...
                conn.execute(
                    f"MERGE (ex:Exception {{id: '{ex_id}', type: '{ex_type}', message: '{ex_msg}', timestamp: timestamp('{timestamp.split('.')[0]}')}})"
                )
                conn.execute(
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        main_layout.setSpacing(8)  # 전체 위젯 배치 간격 축소로 밀집도 향상

        top_bar = QHBoxLayout()
        # [변경 지점] 상태 변화가 누적 표현될 기본 업로드 버튼
        ...
        top_report_box.setSpacing(2)  # 타이틀 레이블과 텍스트 창 간의 컴팩트 조절

        # [요구사항 반영] 타이틀 영역의 높이를 글씨에 딱 맞춰 최소화 조절
        title_lbl = QLabel(
            "<b>📝 인메모리 마이닝 기반 장애 정밀 요약 보고서 (Post-Mortem Report)</b>"
        )
        title_lbl.setFixedHeight(
            title_lbl.fontMetrics().height() + 4
        )  # 텍스트 높이 + 약간의 패딩 보정
    ...
    def upload_log(self):
        ...
        if file_path:
            # [요구사항 반영] 분석 중 상태를 버튼에서 다이렉트로 표현
    ...
    def on_parse_finished(self):
        ...
        # [요구사항 반영] 완료 피드백을 분석 버튼의 레이블 텍스트로 치환 적용
